...
import { AnalysisData, View } from '../types';
import { ShieldAlert, ShieldCheck, Zap, TrendingUp, Lock, CheckCircle } from 'lucide-react';
...
const Insights: React.FC<InsightsProps> = ({ analysis, isPro, onUpgrade }) => {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      ...
      {/* Pricing Deep Dive */}
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-8 shadow-sm transition-colors mb-8">
      ...
      {/* Advanced Reports (Chart Section) */}
      <div className="relative">
        <div className={`grid grid-cols-1 md:grid-cols-2 gap-8 ${!isPro ? 'filter blur-sm select-none' : ''}`}>
           {/* Salary Chart */}
        ...
        {/* Pro Lock Overlay */}
        {!isPro && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/50 dark:bg-slate-950/50 backdrop-blur-sm z-10 rounded-2xl border border-slate-200 dark:border-slate-800">
              <div className="bg-indigo-600 p-3 rounded-full mb-4 shadow-lg shadow-indigo-500/30">
                  <Lock className="text-white w-6 h-6" />
              </div>
              <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Unlock Detailed Market Data</h3>
              <p className="text-slate-600 dark:text-slate-300 mb-6 text-center max-w-md">
                  See how your salary compares to global averages and which skills are trending in your niche.
              </p>
              <button 
                onClick={onUpgrade}
                className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-6 py-3 rounded-xl font-bold hover:scale-105 transition-transform"
              >
                Upgrade to Pro
              </button>
          </div>
